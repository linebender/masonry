...
impl<'a, 'b, W: Widget> PortalMut<'a, 'b, W> {
    ...
    where
        W: StoreInWidgetMut,
    {
        self.state.get_mut(&mut self.widget.child)
    }

    pub fn horizontal_scrollbar_mut(&mut self) -> WidgetMut<'_, 'b, ScrollBar> {
        self.state.get_mut(&mut self.widget.scrollbar_horizontal)
    }

    pub fn vertical_scrollbar_mut(&mut self) -> WidgetMut<'_, 'b, ScrollBar> {
        self.state.get_mut(&mut self.widget.scrollbar_vertical)
    }
    ...
    pub fn set_viewport_pos(&mut self, position: Point) -> bool {
        let portal_size = self.state.widget_state.layout_rect().size();
    ...
    pub fn pan_viewport_to(&mut self, target: Rect) -> bool {
        let viewport =
            Rect::from_origin_size(self.widget.viewport_pos, self.state.widget_state.size);
